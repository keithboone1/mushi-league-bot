...
export async function loadTeamSheet(teamId, season) {
  ...
  const playerQuery =
    "SELECT roster.dropped_week, player.name, player.id, pstat.wins, pstat.act_wins, pstat.losses, pstat.act_losses, pstat.ties, pstat.star_points, pstat.stars, role.name AS role FROM roster \
