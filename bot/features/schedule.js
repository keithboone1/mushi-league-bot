...
async function postSchedulingForMatchup(scheduleChannel, pairingSet) {
  let content = `${roleMention(
    pairingSet[0].leftTeamSnowflake
  )} vs ${roleMention(
    pairingSet[0].rightTeamSnowflake
  )} scheduled times:\n\n`.concat(
    pairingSet
      .map(
        (pairing) =>
...
export async function changeScheduledPlayer(
  scheduleMessageId,
  replacedPlayerSnowflake,
  newPlayerSnowflake
) {
  const scheduleMessage = await getScheduleMessage(scheduleMessageId);
  const newScheduleMessage = scheduleMessage.content
    .replace(RegExp(`^(.*${replacedPlayerSnowflake}.*>:).*$`, "m"), `$1`)
    .replace(replacedPlayerSnowflake, newPlayerSnowflake);
  await scheduleMessage.edit(newScheduleMessage);
}

async function getScheduleMessage(scheduleMessageId) {
  const scheduleChannel = await channels.fetch(process.env.scheduleChannelId);
  return await scheduleChannel.messages.fetch({
    message: scheduleMessageId,
    force: true,
  });
}

export async function setScheduledTime(
  playerSnowflake,
  scheduleMessageId,
  dateString,
  pairingId,
  ...
  const scheduleMessage = await getScheduleMessage(scheduleMessageId);
  const newScheduleMessage = scheduleMessage.content.replace(
    RegExp(`^(.*${playerSnowflake}.*>:).*$`, "m"),
    `$1 ${dateString}`
  );
  await scheduleMessage.edit(newScheduleMessage);
