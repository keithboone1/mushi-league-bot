...
import {
  loadPairingStats,
  savePredictionsMessageId,
} from "../../database/pairing.js";
import {
  loadWeeklyTopTenInPredictions,
  loadCumulativeTopTenInPredictions,
  savePredictionsToDatabase,
} from "../../database/prediction.js";

export async function postPredictions(groupedPairings) {
  const predictionsChannel = await channels.fetch(
    process.env.predictionsChannelId,
  );
...
async function postPredictionsForMatchup(predictionsChannel, pairingSet) {
  ...
  await sendPredictionMessage(
    predictionsChannel,
    headerMessage,
    pairingSet[0].leftEmoji,
    pairingSet[0].rightEmoji,
    "matchup",
    pairingSet[0].matchup,
  );
...
export async function changePredictionsPlayer(
  predictionsMessageId,
  replacedPlayerName,
  newPlayerName,
) {
  const predictionsMessage = await getPredictionsMessage(predictionsMessageId);

  const newPredictionContent = predictionsMessage.content.replace(
    replacedPlayerName,
    newPlayerName,
  );
...
async function getPredictionsMessage(predictionsMessageId) {
  ...
  return await predictionsChannel.messages.fetch({
    message: predictionsMessageId,
    force: true,
  });
}
...
export async function savePredictions(
  ...
  const predictions = await mapReactionsToPlayers(
    leftPlayer,
    leftEmoji,
    rightPlayer,
    rightEmoji,
...
async function mapReactionsToPlayers(
  leftPlayer,
  leftEmoji,
  rightPlayer,
  rightEmoji,
  ...
  const doublePredicters = new Set(
    predictions[leftPlayer].filter((reacter) =>
      predictions[rightPlayer].includes(reacter),
    ),
  );
  predictions[leftPlayer] = predictions[leftPlayer].filter(
    (reacter) => !doublePredicters.has(reacter),
  );
  predictions[rightPlayer] = predictions[rightPlayer].filter(
    (reacter) => !doublePredicters.has(reacter),
  );
...
async function addAutomatedPredictions(
  pairingId,
  predictions,
  leftPlayer,
  rightPlayer,
) {
  predictions[leftPlayer].push(process.env.leftSideLeonardId);

  predictions[rightPlayer].push(process.env.rightSideRonaldId);

  if (Math.random() < 0.5) {
    predictions[leftPlayer].push(process.env.theApeId);
  } else {
    predictions[rightPlayer].push(process.env.theApeId);
  }

  if (predictions[leftPlayer].length > predictions[rightPlayer].length) {
    predictions[leftPlayer].push(process.env.bandwagonBrandonId);
  } else if (predictions[leftPlayer].length < predictions[rightPlayer].length) {
    predictions[rightPlayer].push(process.env.bandwagonBrandonId);
  ...
  const { leftStars, leftStarPoints, rightStars, rightStarPoints } =
    await loadPairingStats(pairingId);

  if (leftStarPoints > rightStarPoints) {
    predictions[leftPlayer].push(process.env.starPointsSammyId);
  } else if (leftStarPoints < rightStarPoints) {
    predictions[rightPlayer].push(process.env.starPointsSammyId);
  } else if (Math.random() < 0.5) {
    predictions[leftPlayer].push(process.env.starPointsSammyId);
  } else {
    predictions[rightPlayer].push(process.env.starPointsSammyId);
  ...
  if (leftStars > rightStars) {
    predictions[leftPlayer].push(process.env.starRatingSteveId);
  } else if (leftStars < rightStars) {
    predictions[rightPlayer].push(process.env.starRatingSteveId);
  } else if (Math.random() < 0.5) {
    predictions[leftPlayer].push(process.env.starRatingSteveId);
  } else {
    predictions[rightPlayer].push(process.env.starRatingSteveId);
...
export async function updatePrediction(
  pairingPredictionsMessageId,
  matchupPredictionsMessageId,
  act,
  dead,
  winnerOnLeft,
) {
...
async function updatePairingPrediction(
  pairingPredictionsMessageId,
  act,
  dead,
  winnerOnLeft,
) {
  const pairingPredictionsMessage = await getPredictionsMessage(
    pairingPredictionsMessageId,
  );
...
async function updateMatchupPrediction(
  matchupPredictionsMessageId,
  winnerOnLeft,
) {
  const matchupPredictionsMessage = await getPredictionsMessage(
    matchupPredictionsMessageId,
  );
  const score = matchupPredictionsMessage.content.substring(
    matchupPredictionsMessage.content.length - 3,
    matchupPredictionsMessage.content.length,
  );
  ...
  const newMatchupPredictionsContent = matchupPredictionsMessage.content
    .substring(0, matchupPredictionsMessage.content.length - 3)
    .concat(newScore);
  await matchupPredictionsMessage.edit(newMatchupPredictionsContent);
}

export async function resetPredictionWinner(
  pairingPredictionsMessageId,
  matchupPredictionsMessageId,
  winnerWasOnLeft,
  pairingWasDead,
...
async function resetPairingPrediction(
  pairingPredictionsMessageId,
  leftPlayerName,
  rightPlayerName,
) {
  const predictionsMessage = await getPredictionsMessage(
    pairingPredictionsMessageId,
  );
...
async function resetMatchupPrediction(
  matchupPredictionsMessageId,
  winnerWasOnLeft,
) {
  const matchupPredictionsMessage = await getPredictionsMessage(
...
export async function postPredictionStandings(season, week) {
  ...
  await sendTopTenMessage(
    weeklyTopTen,
    bold(`Weekly top ten in predictions for ${weekName(week)}:`),
  );

  if (week > 1) {
    const cumulativeTopTen = await loadCumulativeTopTenInPredictions(
      season,
      week,
    );
    await sendTopTenMessage(
      cumulativeTopTen,
      bold(`Cumulative top ten in predictions as of ${weekName(week)}:`),
    );
  }
}
...
async function sendTopTenMessage(topTen, header) {
  const message = header.concat(
    ...
    topTen
      .map(
        (val, index) =>
          `${index + 1}. ${userMention(val.predictor_snowflake)} (${val.correctPredictions})`,
      )
      .join("\n"),
  );
