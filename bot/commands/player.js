  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addNumberOption((option) =>
          option.setName("stars").setDescription("Star rating of player"),
        ),
    )
    .addSubcommand((subcommand) =>
      subcommand
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addUserOption((option) =>
          option.setName("player").setDescription("Player"),
        )
        .addStringOption((option) =>
          option.setName("player_name").setDescription("Player username"),
        ),
    )
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addUserOption((option) =>
          option.setName("player").setDescription("Player to add"),
        )
        .addStringOption((option) =>
          option.setName("player_name").setDescription("Player username"),
        ),
...
async function addPlayer(interaction) {
  ...
  function verifier(data) {
    ...
    if (existingPlayer) {
      failures.push(
        `${userMention(playerSnowflake)} is already in the pool! To adjust their name, use /player rename. To adjust their rating, use /player rate.`,
      );
    }
...
async function renamePlayer(interaction) {
  async function dataCollector(interaction) {
    ...
    if (!existingPlayer) {
      return {
        failure: `${player} is not in the pool; use /player add instead`,
      };
    }
...
async function ratePlayer(interaction) {
  ...
  function verifier(data) {
    ...
    if (existingPlayer.stars === stars) {
      failures.push(
        `${userMention(playerSnowflake)} is already rated ${stars}.`,
      );
    }

    if (existingPlayer.stars && existingPlayer.stars !== stars) {
      prompts.push(
        `${userMention(playerSnowflake)} is already rated ${existingPlayer.stars}. Do you want to change their rating to ${stars}?`,
      );
    }
...
async function assignPlayer(interaction) {
  async function dataCollector(interaction) {
    ...
    if (!existingPlayer) {
      return {
        failure: `${player} is not in the pool; use /player add first.`,
      };
    }

    const existingLeader =
      newRole.name === "Captain" || newRole.name === "Coach"
  ...
  function verifier(data) {
    ...
    if (
      existingPlayer.teamSnowflake === newTeam.id &&
      existingPlayer.roleSnowflake === newRole.id
    ) {
      failures.push(
        `${player} is already assigned to ${newTeam} as a ${newRole}`,
      );
    }

    if (existingPlayer.stars === null && newRole.name !== "Coach") {
      failures.push(
        `${player} needs a star rating before being made a ${newRole}. Use /player rate.`,
      );
    }
    ...
    if (
      ...
      prompts.push(
        `${player} is already on ${newTeam} but will be moved from ${roleMention(existingPlayer.roleSnowflake)} to ${newRole}.`,
      );
    }

    if (
      existingPlayer.teamSnowflake &&
      existingPlayer.teamSnowflake !== newTeam.id
    ) {
      prompts.push(
        `${player} is on ${roleMention(existingPlayer.teamSnowflake)} but will be moved to ${newTeam}.`,
      );
    }

    if (
      existingPlayer.roleName &&
      existingPlayer.roleName !== "Player" &&
      (newRole.name !== existingPlayer.roleName ||
        existingPlayer.teamSnowflake !== newTeam.id)
    ) {
      prompts.push(
        `${player} was ${roleMention(existingPlayer.teamSnowflake)}'s ${roleMention(existingPlayer.roleSnowflake)}. This team will be without a ${roleMention(existingPlayer.roleSnowflake)}.`,
      );
    }

    if (existingLeader) {
      prompts.push(
        `${userMention(existingLeader.discord_snowflake)} is already ${newTeam}'s ${newRole}. Teams shouldn't have two of these.`,
      );
    }
  ...
  async function onConfirm(data) {
    ...
    roles = roles.filter(
      (role) =>
        role !== existingPlayer.roleSnowflake &&
        role !== existingPlayer.teamSnowflake,
    );
...
async function dropPlayer(interaction) {
  ...
  function verifier(data) {
    ...
    if (!existingPlayer.teamSnowflake) {
      failures.push(
        `${userMention(existingPlayer.discord_snowflake)} is already not on a team.`,
      );
    }

    if (
      existingPlayer.roleName === "Captain" ||
      existingPlayer.roleName === "Coach"
    ) {
      prompts.push(
        `${userMention(existingPlayer.discord_snowflake)} was ${roleMention(existingPlayer.teamSnowflake)}'s ${existingPlayer.roleName}. This team will be without a ${existingPlayer.roleName}.`,
      );
    }
...
async function setPlayerInactive(interaction) {
  ...
  function verifier(data) {
    ...
    if (existingPlayer.teamSnowflake) {
      prompts.push(
        `${userMention(existingPlayer.discord_snowflake)} was on ${roleMention(existingPlayer.teamSnowflake)}. They will be dropped.`,
      );
    }
...
async function setPlayerActive(interaction) {
  async function dataCollector(interaction) {
    ...
    if (!existingPlayer) {
      return {
        failure: `${player || existingPlayer} is not in the pool; use /player add instead.`,
      };
    }

    return {
      playerSnowflake: existingPlayer.discord_snowflake,
      existingPlayer,
    };
  }
