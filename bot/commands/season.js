...
import {
  ...
  saveAdvanceWeek,
  saveWinner,
} from "../../database/season.js";
import { saveNewWeeks } from "../../database/week.js";
import {
  saveMatchRoomMessageId,
  saveOneNewMatchup,
  loadAllMatchups,
  loadMatchupsMissingLineups,
  loadOldPairingMessage,
} from "../../database/matchup.js";
import {
  saveInitialStandings,
  loadStandingWeeksSoFar,
  loadStandings,
  saveStandingsUpdate,
  loadTopTeams,
...
import {
  ...
  loadPlayersOnTeamInStarOrder,
  loadAllActivePlayers,
} from "../../database/player.js";
import {
  loadAllPairings,
  loadAllPairingResults,
  loadOpenPairings,
...
async function makeWinnerRole(winningTeamId, winningTeamSnowflake) {
  const color = (await mushiLeagueGuild.roles.fetch(winningTeamSnowflake))
    .hexColor;
...
async function autoGenerateLineup(matchup, userForAutoLineups) {
  ...
  const lineup = (
    await loadPlayersOnTeamInStarOrder(matchup.submittingTeamId)
  ).slice(0, slots);
...
async function unpinOldPairingMessage(room, matchRoom) {
  try {
    const oldPairingMessageId = (await loadOldPairingMessage(room))
      .channel_message;
